...
export function debugFetchEnvironment(): FetchDebugInfo {
  ...
  if (window.fetch) {
    ...
    } else if (fetchString.includes('fullstory') || fetchString.includes('fs.js')) {
...
export function createRobustFetch(): typeof fetch {
  return async function robustFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
