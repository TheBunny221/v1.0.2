...
export function useScreenReader() {
  ...
  const announceSuccess = useCallback((message?: string) => {
    const successMessage = message || translations?.messages?.operationSuccess || 'Operation completed successfully';
    announce(successMessage, 'polite');
  }, [announce, translations]);

  const announceError = useCallback((message?: string) => {
    const errorMessage = message || translations?.messages?.error || 'An error occurred';
...
export function useSkipLinks() {
  ...
  const addSkipLink = useCallback((target: string, label: string) => {
    ...
    link.className = 'skip-link sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-50 focus:bg-primary focus:text-primary-foreground focus:px-4 focus:py-2 focus:rounded';
...
export function useAriaAttributes() {
  ...
  const setAriaDescribedBy = useCallback((element: HTMLElement | null, ids: string | string[]) => {
    if (element) {
  ...
  const setAriaChecked = useCallback((element: HTMLElement | null, checked: boolean | 'mixed') => {
    if (element) {
...
export function useColorContrast() {
  ...
  const isAACompliant = useCallback((foreground: string, background: string): boolean => {
    return checkContrast(foreground, background) >= 4.5;
  }, [checkContrast]);

  const isAAACompliant = useCallback((foreground: string, background: string): boolean => {
