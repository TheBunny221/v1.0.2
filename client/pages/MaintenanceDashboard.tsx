...
const MaintenanceDashboard: React.FC = () => {
  ...
  const assignedComplaints: AssignedComplaint[] = [
    {
      id: 'CMP-2024-001',
      type: 'Water Supply',
      description: 'No water supply for the past 3 days in Green Valley Society. Multiple residents affected.',
  ...
  const metrics = {
    ...
    dueToday: 1,
    overdue: assignedComplaints.filter(c => new Date(c.dueDate) < new Date()).length,
  };

  const slaComplaints = assignedComplaints.map(complaint => {
    ...
    return {
      ...complaint,
      hoursLeft,
      slaStatus: hoursLeft < 0 ? 'overdue' : hoursLeft < 24 ? 'warning' : 'ontime'
    };
                    ...
                    <TableRow key={complaint.id}>
                      ...
                      <TableCell>
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">{complaint.ward}</div>
                        </div>
                      </TableCell>
                      <TableCell>
                              ...
                              <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                  <div>
                                    ...
                                    <p className="text-sm">{complaint.assignedDate}</p>
                                  </div>
                                  <div>
                                    <Label>Due Date</Label>
                                    <p className="text-sm">{complaint.dueDate}</p>
                                  </div>
                                </div>
                                  ...
                                  <div>
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-1">
                                      {complaint.attachments.map((file, idx) => (
                                        <Badge key={idx} variant="secondary">{file}</Badge>
                                      ))}
                                    </div>
                                  </div>
                                )}
                          ...
                          <Dialog>
                            ...
                            <DialogContent className="max-w-2xl">
                              ...
                              <div className="space-y-4">
                                ...
                                <div>
                                  <Label htmlFor="updateRemarks">Update Remarks</Label>
                                  <Textarea
                                    id="updateRemarks"
                                    value={updateRemarks}
                                  ...
                                  <div>
                                    ...
                                    <div className="border-2 border-dashed border-border rounded-lg p-4 text-center">
                                      ...
                                      {beforePhotos.length > 0 && (
                                        <p className="text-sm mt-2">{beforePhotos.length} file(s) selected</p>
                                      )}
                                    </div>
                                  </div>
                                  ...
                                  <div>
                                    ...
                                    <div className="border-2 border-dashed border-border rounded-lg p-4 text-center">
                                      ...
                                      {afterPhotos.length > 0 && (
                                        <p className="text-sm mt-2">{afterPhotos.length} file(s) selected</p>
                                      )}
                                    </div>
                                  </div>
                                </div>
                ...
                {slaComplaints.map((complaint) => (
                  <div key={complaint.id} className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <div className="flex flex-col items-end space-y-1">
                      <StatusChip status={complaint.status} />
                      <span className={`text-sm font-medium ${getSlaStatusColor(complaint.slaStatus)}`}>
        ...
        <TabsContent value="completed">
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">Completed Complaints</h3>
              <p className="text-muted-foreground">
