...
import { setLanguage, selectLanguage, selectTranslations } from '../store/slices/languageSlice';
...
export const LanguageSwitcher: React.FC<LanguageSwitcherProps> = ({
  ...
  const handleLanguageChange = async (newLanguage: LanguageCode) => {
    ...
    try {
      ...
      if (isAuthenticated) {
        await dispatch(updateUserPreferences({ language: newLanguage })).unwrap();
      }
  ...
  if (variant === 'select') {
    return (
      <div className={cn("relative", className)}>
        ...
        >
          <SelectTrigger className={cn(sizeClasses[size], "w-auto min-w-[120px]")}>
            <SelectValue>
              {isChanging ? (
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              ...
              size={size}
              disabled={isChanging || isLoading}
              className={cn("w-auto min-w-[120px] justify-start", sizeClasses[size])}
            >
              {isChanging ? (
                <div className="flex items-center space-x-2">
                  <Loader2 className="h-4 w-4 animate-spin" />
...
export const useLanguage = () => {
  ...
  const formatDate = (date: Date, options?: Intl.DateTimeFormatOptions) => {
    const locale = currentLanguage === 'hi' ? 'hi-IN' : 
...
export const TextDirectionProvider: React.FC<TextDirectionProviderProps> = ({ children }) => {
...
export const Translate: React.FC<TranslateProps> = ({ path, values, fallback }) => {
  const translations = useAppSelector(selectTranslations);
