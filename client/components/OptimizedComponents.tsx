...
import { useVirtualScrolling, useLazyImage, useIntersectionObserver } from '../hooks/usePerformance';
...
const VirtualListItem = memo(({ index, style, data, renderItem }: {
  index: number;
  style: React.CSSProperties;
  data: { items: VirtualListItem[]; renderItem: VirtualListProps['renderItem'] };
  ...
  return (
    <div style={style}>
      {data.renderItem({ index, style, data: item })}
    </div>
  );
}, areEqual);
...
export const OptimizedCardGrid = memo(<T,>({
  items,
  renderCard,
  columns = 3,
  gap = 16,
...
export const LazySection: React.FC<LazySectionProps> = memo(({
  ...
  return (
    ...
    >
      {isIntersecting ? children : (fallback || <Skeleton className="w-full h-full" />)}
    </div>
  );
});
    ...
    <div className={cn("relative", className)}>
      ...
      {isOpen && (
        ...
        >
          {shouldVirtualize ? (
            <VirtualList
              items={options.map((option, index) => ({ id: option.value, data: option }))}
              itemHeight={40}
