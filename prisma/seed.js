...
async function main() {
  ...
  try {
    ...
    const wardsData = [
      { name: 'Ward 1 - Fort Kochi', description: 'Historic Fort Kochi area including Chinese fishing nets' },
      { name: 'Ward 2 - Mattancherry', description: 'Mattancherry Palace and spice markets area' },
      { name: 'Ward 3 - Ernakulam South', description: 'Commercial district and shopping areas' },
      { name: 'Ward 4 - Kadavanthra', description: 'Residential area with IT companies' },
      { name: 'Ward 5 - Panampilly Nagar', description: 'Upscale residential and commercial area' },
      { name: 'Ward 6 - Marine Drive', description: 'Waterfront promenade and business district' },
      { name: 'Ward 7 - Willingdon Island', description: 'Port area and industrial zone' },
    ...
    for (const ward of createdWards) {
      ...
      for (let i = 0; i < 3; i++) { // Create 3 sub-zones per ward
    ...
    const departments_list = ['Public Works', 'Water Supply', 'Electricity', 'Waste Management'];
    ...
    const citizens = [];
    const citizenData = [
      { name: 'Rajesh Kumar', email: 'rajesh.kumar@email.com', phone: '+91-9876540001' },
      { name: 'Priya Nair', email: 'priya.nair@email.com', phone: '+91-9876540002' },
      { name: 'Mohammed Ali', email: 'mohammed.ali@email.com', phone: '+91-9876540003' },
      { name: 'Sunitha Menon', email: 'sunitha.menon@email.com', phone: '+91-9876540004' },
      { name: 'Ravi Krishnan', email: 'ravi.krishnan@email.com', phone: '+91-9876540005' }
    ];
    ...
    const complaintTypes = [
      'WATER_SUPPLY', 'ELECTRICITY', 'ROAD_REPAIR', 'WASTE_MANAGEMENT', 
      'STREET_LIGHTING', 'DRAINAGE', 'PUBLIC_TOILET', 'TREE_CUTTING'
    ];
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const complaint = await prisma.complaint.create({
        data: {
          ...
          submittedOn: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000), // Random date within last 30 days
    ...
    for (let i = 0; i < 10; i++) {
      ...
      await prisma.serviceRequest.create({
        data: {
          ...
          contactPhone: randomCitizen.phoneNumber,
          submittedById: randomCitizen.id,
          submittedOn: new Date(Date.now() - Math.random() * 20 * 24 * 60 * 60 * 1000), // Random date within last 20 days
    ...
    console.log('Maintenance: maintenance1@cochinsmartcity.gov.in / maintenance123');
