...
async function exportData() {
  ...
  try {
    ...
    const serviceStatusLogs = await sqlitePrisma.serviceRequestStatusLog.findMany();
    ...
    function generateInserts(tableName, data, fields) {
      ...
      for (const record of data) {
        const values = fields.map(field => escapeSqlValue(record[field])).join(', ');
        statements.push(`INSERT INTO "${tableName}" (${fieldNames}) VALUES (${values});`);
      }
    ...
    sqlStatements.push(...generateInserts('wards', wards, ['id', 'name', 'description', 'isActive', 'createdAt', 'updatedAt']));
    sqlStatements.push('');

    sqlStatements.push('-- Insert departments');
    sqlStatements.push(...generateInserts('departments', departments, ['id', 'name', 'description', 'headUserId', 'isActive', 'createdAt', 'updatedAt']));
    sqlStatements.push('');

    sqlStatements.push('-- Insert users');
    sqlStatements.push(...generateInserts('users', users, ['id', 'email', 'fullName', 'phoneNumber', 'password', 'role', 'wardId', 'department', 'language', 'avatar', 'isActive', 'lastLogin', 'joinedOn', 'createdAt', 'updatedAt']));
    sqlStatements.push('');

    sqlStatements.push('-- Insert sub_zones');
    sqlStatements.push(...generateInserts('sub_zones', subZones, ['id', 'name', 'wardId', 'description', 'isActive', 'createdAt', 'updatedAt']));
    sqlStatements.push('');

    sqlStatements.push('-- Insert system_config');
    sqlStatements.push(...generateInserts('system_config', systemConfig, ['id', 'key', 'value', 'description', 'isActive', 'updatedAt']));
    sqlStatements.push('');

    sqlStatements.push('-- Insert complaints');
    sqlStatements.push(...generateInserts('complaints', complaints, ['id', 'complaintId', 'title', 'description', 'type', 'status', 'priority', 'slaStatus', 'wardId', 'subZoneId', 'area', 'landmark', 'address', 'coordinates', 'contactName', 'contactEmail', 'contactPhone', 'isAnonymous', 'submittedById', 'assignedToId', 'resolvedById', 'submittedOn', 'assignedOn', 'resolvedOn', 'closedOn', 'deadline', 'remarks', 'citizenFeedback', 'rating', 'tags', 'createdAt', 'updatedAt']));
    sqlStatements.push('');

    sqlStatements.push('-- Insert status_logs');
    sqlStatements.push(...generateInserts('status_logs', statusLogs, ['id', 'complaintId', 'userId', 'fromStatus', 'toStatus', 'comment', 'timestamp']));
    sqlStatements.push('');

    sqlStatements.push('-- Insert service_requests');
    sqlStatements.push(...generateInserts('service_requests', serviceRequests, ['id', 'title', 'serviceType', 'description', 'status', 'priority', 'wardId', 'area', 'address', 'landmark', 'contactName', 'contactEmail', 'contactPhone', 'submittedById', 'assignedToId', 'submittedOn', 'preferredDateTime', 'assignedOn', 'expectedCompletion', 'completedOn', 'remarks', 'citizenFeedback', 'rating', 'createdAt', 'updatedAt']));
    sqlStatements.push('');

    sqlStatements.push('-- Insert service_request_status_logs');
    sqlStatements.push(...generateInserts('service_request_status_logs', serviceStatusLogs, ['id', 'serviceRequestId', 'userId', 'fromStatus', 'toStatus', 'comment', 'timestamp']));
    sqlStatements.push('');

    sqlStatements.push('-- Insert notifications');
    sqlStatements.push(...generateInserts('notifications', notifications, ['id', 'userId', 'complaintId', 'serviceRequestId', 'type', 'title', 'message', 'isRead', 'sentAt', 'readAt']));
    sqlStatements.push('');

    sqlStatements.push('-- Insert messages');
    sqlStatements.push(...generateInserts('messages', messages, ['id', 'complaintId', 'sentById', 'receivedById', 'content', 'isInternal', 'sentAt', 'readAt']));
    sqlStatements.push('');

    sqlStatements.push('-- Insert attachments');
    sqlStatements.push(...generateInserts('attachments', attachments, ['id', 'complaintId', 'fileName', 'originalName', 'mimeType', 'size', 'url', 'uploadedAt']));
    sqlStatements.push('');

    sqlStatements.push('-- Insert otp_sessions');
    sqlStatements.push(...generateInserts('otp_sessions', otpSessions, ['id', 'userId', 'email', 'phoneNumber', 'otpCode', 'purpose', 'isVerified', 'expiresAt', 'createdAt', 'verifiedAt']));
    sqlStatements.push('');

    sqlStatements.push('-- Insert reports');
    sqlStatements.push(...generateInserts('reports', reports, ['id', 'name', 'type', 'filters', 'data', 'generatedBy', 'generatedAt']));
    sqlStatements.push('');
    ...
    sqlStatements.push('-- SELECT setval(pg_get_serial_sequence(\'"table_name"\', \'id\'), (SELECT MAX(id) FROM "table_name"));');
    sqlStatements.push('');
    ...
    console.log('3. Run: psql -U username -d database_name -f postgresql_migration.sql');
