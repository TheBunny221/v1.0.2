...
async function validateEnvironment() {
  ...
  if (missingVars.length > 0) {
    ...
    log('yellow', '💡 Please check your .env file or environment configuration');
    return false;
  }
...
async function checkDatabaseConnection() {
  ...
  try {
    ...
    if (error.message.includes('Can\'t reach database server')) {
      ...
      log('yellow', '   1. Ensure PostgreSQL is running on the specified host and port');
      log('yellow', '   2. Check if the host and port in DATABASE_URL are correct');
      log('yellow', '   3. Verify firewall settings allow connections');
      log('yellow', '   4. For cloud databases, check IP whitelist and connection limits');
    } else if (error.message.includes('password authentication failed')) {
      log('yellow', '💡 Authentication issue:');
      log('yellow', '   1. Check username and password in DATABASE_URL');
      log('yellow', '   2. Verify the database user exists and has proper permissions');
...
async function checkMigrationStatus() {
  ...
  try {
    ...
    try {
      ...
      if (!result[0].exists) {
        log('yellow', '⚠️ No migration history found - this appears to be a fresh database');
    ...
    if (existingTables.length === tables.length) {
      ...
      log('yellow', `⚠️ Missing tables: ${tables.filter(t => !existingTables.includes(t)).join(', ')}`);
      log('cyan', '💡 Run: npm run db:migrate:deploy');
    }
...
async function checkDataSeeding() {
  ...
  try {
    ...
    if (userCount === 0) {
      ...
      log('cyan', '💡 Run: npm run seed:prod (for production) or npm run seed:dev (for development)');
    } else {
  ...
  try {
    ...
    if (complaintStats.length > 0) {
      ...
      complaintStats.forEach(stat => {
        log('cyan', `${stat.status.padEnd(15)}: ${stat._count.toLocaleString()}`);
      });
    }
