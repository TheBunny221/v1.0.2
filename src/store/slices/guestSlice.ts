...
const initialState: GuestState = {
  ...
  isLoading: false,
  error: null,
  guestComplaintIds: JSON.parse(localStorage.getItem('guestComplaintIds') || '[]'),
};

// Async thunks
export const sendOtpForGuest = createAsyncThunk(
  'guest/sendOtp',
  async (params: { email: string; complaintData: GuestComplaintData }, { rejectWithValue }) => {
    try {
...
export const verifyOtpAndSubmitComplaint = createAsyncThunk(
  ...
  async (params: { otp: string; sessionId: string }, { getState, rejectWithValue }) => {
    try {
      ...
      if (complaintData.latitude) formData.append('latitude', complaintData.latitude.toString());
      if (complaintData.longitude) formData.append('longitude', complaintData.longitude.toString());
      ...
      if (!complaintResponse.ok) {
        return rejectWithValue(complaintResponseData.message || 'Failed to submit complaint');
      }
...
export const trackGuestComplaint = createAsyncThunk(
  ...
  async (params: { complaintId: string; email: string; mobile: string }, { rejectWithValue }) => {
    try {
  ...
  reducers: {
    ...
    addGuestComplaintId: (state, action: PayloadAction<string>) => {
      if (!state.guestComplaintIds.includes(action.payload)) {
        ...
        localStorage.setItem('guestComplaintIds', JSON.stringify(state.guestComplaintIds));
      }
    },
    removeGuestComplaintId: (state, action: PayloadAction<string>) => {
      state.guestComplaintIds = state.guestComplaintIds.filter(id => id !== action.payload);
      localStorage.setItem('guestComplaintIds', JSON.stringify(state.guestComplaintIds));
    },
...
export const selectIsOtpSent = (state: { guest: GuestState }) => state.guest.isOtpSent;
export const selectIsOtpVerifying = (state: { guest: GuestState }) => state.guest.isOtpVerifying;
export const selectIsOtpVerified = (state: { guest: GuestState }) => state.guest.isOtpVerified;
export const selectOtpSessionId = (state: { guest: GuestState }) => state.guest.otpSessionId;
export const selectOtpExpiresAt = (state: { guest: GuestState }) => state.guest.otpExpiresAt;
export const selectPendingComplaint = (state: { guest: GuestState }) => state.guest.pendingComplaint;
export const selectIsSubmittingComplaint = (state: { guest: GuestState }) => state.guest.isSubmittingComplaint;
export const selectSubmittedComplaintId = (state: { guest: GuestState }) => state.guest.submittedComplaintId;
export const selectGuestLoading = (state: { guest: GuestState }) => state.guest.isLoading;
export const selectGuestError = (state: { guest: GuestState }) => state.guest.error;
export const selectGuestComplaintIds = (state: { guest: GuestState }) => state.guest.guestComplaintIds;
